...
function getBills(){
    ...
    $.ajax({
        ...
        url :"{{ route('dashboard.filter-budget-invoice') }}",
        success: function(listData){
            ...
            if(listData.length > 0){
                listData.forEach(function(invoice){
                    var url = '{{ route("budget-invoice.show", ":id") }}';
                    url = url.replace(':id', invoice.budget_invoice_id);
                    var row = table.row.add([
                        invoice.budget_invoice_name,
                        invoice.budget_invoice_type_name,
                        invoice.budget_name,
                        invoice.user_name,
